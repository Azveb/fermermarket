...
export default function AdminEditProductPage({ params }) {
  ...
  useEffect(() => {
    ...
    .then(([catData, prodData]) => {
      ...
      // We don't auto-expand the category dropdowns in edit mode for simplicity, 
      // but we set categoryId. If they change it, they will use the dropdowns.
      // To pre-select, we'd need to traverse the tree, which is complex.
    })
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">Məhsulu Redaktə Et</h1>
          <p className="text-gray-500 text-sm">ID: {id}</p>
        </div>
      </div>

      {success && <div className="bg-green-50 text-green-700 p-4 rounded-xl mb-6 font-bold">Dəyişikliklər yadda saxlanıldı! Yönləndirilirsiniz...</div>}
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Məhsul adı</label>
          ...
          <div>
            ...
            <select className="input-field" value={form.status} onChange={e => setForm({...form, status: e.target.value})}>
              ...
              <option value="PENDING_REVIEW">Gözləyir (Təsdiq)</option>
              <option value="REJECTED">Rədd edilib</option>
              <option value="SOLD">Satıldı</option>
              <option value="EXPIRED">Bitib (Vaxtı keçib)</option>
            </select>
          </div>
        </div>
